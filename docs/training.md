...
Training accepts completed TACO `.tortilla` and `.taco` files. CSV manifests, loose raster directories, array/tensor files, synthetic samples, and on-the-fly degradations are deliberately not part of this pipeline. Both supported datasets use the `tacoreader.v1` adapter for their container layout.
...
| Key | Shape | Meaning |
| --- | --- | --- |
| `image` | `4 × H × W` | RGB HR target plus upsampled native-resolution B8 |
...
There is an important scientific limitation: `hrharm` contains RGB only. To retain the current model's four-channel RGB-NIR checkpoint contract, the HR tensor's fourth channel is a bilinear enlargement of native LR B8. This preserves B8 semantics and radiometry, but it is not HR NIR ground truth and provides no learned high-resolution NIR supervision. Do not interpret NIR reconstruction or SR metrics as validation against independently observed HR NIR.
